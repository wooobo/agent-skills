...
- 2026-08-11 | 호출자 pane 을 쪼개는 배치를 폐기 | 오케스트레이터가 자기 pane 만 쪼갤 수 있어 결과가 계단이 되고, 워커가 늘수록 **승인 화면을 읽어야 하는 쪽**의 시야가 좁아진다 | 워커는 전용 탭에 격자로 넣는다. 탭당 6개, 배치는 워커 수로 결정(2→세로2, 3→세로3, 4→2x2, 5→2/2/1, 6→3x2), 7번째부터 새 탭. 284x79 에서 전부 하한(80x20) 통과 실측
- 2026-08-11 | 격자를 미리 다 쪼개 두는 이유 | 슬롯이 필요할 때마다 쪼개면 이미 돌고 있는 워커의 pane 을 쪼개게 된다. 그 워커가 승인 다이얼로그를 띄운 순간이면 리사이즈 리드로가 판독을 깬다 | 첫 스폰 때 `--total` 만큼 한 번에 만든다. 대신 슬롯은 탭 생성 시 cwd 를 물려받으므로, 에이전트를 띄우기 전에 `cd` 시키고 `pane process-info` 의 cwd 로 **확인**해야 한다 — 확인 없이 띄우면 워커가 엉뚱한 저장소에서 조용히 작업한다
