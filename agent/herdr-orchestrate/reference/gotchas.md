...
- 2026-08-11 | pane 분할로 워커 3개가 한계 | 오케스트레이터는 자기 pane 만 쪼갤 수 있어 결과가 격자가 아니라 **계단**이다. 284x79 창의 이론상 격자 용량은 9칸인데 계단으로는 3개고, `--ratio` 로 정확히 80칸씩 잘라도 284=80*3+44 라 역시 3개다 | 기하 계산을 개선해도 못 넘는다. `spawn` 이 하한에 걸리면 새 탭으로 폴백한다 — 탭 root pane 은 항상 전체 크기다. `--ratio` 는 원본이 **유지**하는 비율(0.33 → 원본 94/284)
