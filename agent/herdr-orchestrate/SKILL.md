...
kind별 스폰 플래그, 워커 탭 격자 배치, pane_id 파싱, 결과 파일 경로,
...
**워커는 호출자 pane을 쓰지 않는다.** 오케스트레이터 pane은 그대로 두고, 워커는
**전용 탭**에 격자로 들어간다. 자기 pane을 쪼개면 워커가 늘수록 오케스트레이터의
시야가 좁아지는데, 승인 화면을 읽어야 하는 쪽이 좁아지는 건 앞뒤가 안 맞는다.

탭 하나에 **최대 6개**, 배치는 워커 수로 정해진다:
...
| 3 | 세로 3분할 | 95 / 95 / 94 |
| 4 | 2×2 | 142x40 |
| 5 | 2/2/1 | 95x40 · 95x40 · 94x79 |
| 6 | 3×2 | 95x40 |

**7개째부터는 새 탭**이 열린다. 그래서 기하는 더 이상 "몇 개까지 되나"를 정하지 않는다.

격자는 첫 스폰 때 **한 번에 다 쪼개 둔다.** 필요할 때마다 쪼개면 이미 돌고 있는 워커의
pane을 쪼개게 되는데, 하필 그 워커가 승인 다이얼로그를 띄운 순간이면 리사이즈로 화면이
다시 그려져 판독이 깨진다. 그래서 `spawn`에 **`--total <워커 총수>`를 반드시 넘긴다.**

창이 작아 하한(폭 80칸/높이 20줄)을 못 지키면 그 탭에 넣는 수를 줄이고 나머지는 다음
탭으로 보낸다. 하한을 깨느니 탭을 하나 더 여는 게 낫다 — 좁은 pane에서는 승인 UI가
줄바꿈으로 뭉개져 `agent read`로 무엇을 묻는지 판독할 수 없고, 그러면 blocked 대응이
불가능해진다.

**워커 탭은 화면에 안 보인다.** 감시는 `status`/`agent read`로 하면 되지만 정리를 잊기
쉬우니 최종 보고에 반드시 올린다. `collect`가 탭 목록을 따로 찍어준다.
...
- 출력의 `tab=`을 보고 어느 탭에 떴는지 확인하고 **최종 보고에 올린다**
