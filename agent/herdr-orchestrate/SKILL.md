...
kind별 스폰 플래그, 워커 배치(pane 분할 또는 새 탭), pane_id 파싱, 결과 파일 경로,
그리고 run manifest(워커→cwd→pane 매핑). 매번 손으로 조립하면 틀리는 것들이다.

```
orchestrate.mjs spawn   --run <id> --name <n> --kind <k> --cwd <path> [--mode write|read-only] [--layout auto|pane|tab]
...
고정 상한은 없다. 이 워크스페이스가 정하는 두 값의 최소값이다:

1. **독립 작업 디렉토리 수.** 디렉토리가 2개면 워커도 2개다. 이게 진짜 상한이다
2. **`rules.md`의 `max_concurrent`.** 있으면 그것이 최종 상한이다

**pane 기하는 상한이 아니다.** 오케스트레이터는 자기 pane만 쪼갤 수 있어서 반복
분할의 결과가 격자가 아니라 계단이고, 계단은 3개쯤에서 막힌다(284x79 창에서 실측 3개).
그래서 `spawn`은 하한(폭 80칸/높이 20줄)에 걸리면 **새 탭**에 워커를 띄운다.
탭의 pane은 항상 전체 크기라 제약이 사라진다. 즉 기하는 이제 "몇 개까지 되나"가
아니라 **"보이는 곳에 놓을까, 탭으로 보낼까"**만 정한다.

하한을 지키는 이유는 그대로다 — 좁은 pane에서는 승인 UI가 줄바꿈으로 뭉개져
`agent read`로 무엇을 묻는지 판독할 수 없고, 그러면 blocked 대응이 불가능해진다.
안 보이는 탭에 크게 띄우는 편이 낫다.

**탭으로 간 워커는 화면에 안 보인다.** 감시는 `status`/`agent read`로 하면 되지만
정리를 잊기 쉬우니 최종 보고에 반드시 올린다. `collect`가 탭 목록을 따로 찍어준다.

기하가 안 막아주므로 **감시 능력이 실질 상한이다.** 워커가 4개를 넘어가면 스폰
**전에** 대상 목록을 보여주고 사용자에게 확인받는다. 숫자를 임의로 깎지 말고 근거를
보여주고 판단을 넘기는 게 맞다.
...
- 배치는 `--layout auto`(기본)가 알아서 한다. pane이 하한을 못 지키면 새 탭으로 간다.
  출력의 `pane=` / `tab=`을 보고 어디 떴는지 확인하고, 탭이면 최종 보고에 올린다.
  꼭 나란히 보여야 하면 `--layout pane`(자리 없으면 실패), 처음부터 안 보여도 되면
  `--layout tab`
...
- 아직 살아있는 pane 목록, **그리고 이 run 이 만든 탭 목록**. 탭 워커는 화면에 안 보여서
  정리를 잊기 쉽다. 정리 여부를 사용자에게 묻는다
...
- 탭에 띄운 워커도 내가 만든 것이다. 화면에 없다고 보고에서 빠뜨리지 않는다
